...
| SEC-4 | En-têtes de sécurité + TLS | [nginx.conf](../frontend/nginx.conf) : ni CSP, ni `X-Frame-Options`, ni HSTS, ni `server_tokens off`, ni `X-Forwarded-For` vers le back, ni cache-control sur les assets hashés. Aucun scénario HTTPS | M | P1 si public | **en cours** — [spec](superpowers/specs/2026-07-31-sec-4-entetes-securite-tls-design.md). Élargi à la moitié « Dockerfile » de SEC-6 (non-root, `HEALTHCHECK`, cache de couche Maven), qui vit dans les mêmes fichiers |
