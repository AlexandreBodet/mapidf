...
|---|---|---|---|---|---|
| PROD-1 | Perturbations (`disruptions_bulk`) | Le manque le plus criant : on voit des trains, jamais « interruption entre X et Y ». SIRI `general-message` écarté (vide pour le métro, un paramètre par ligne) | M | P1 | **backend fait** (poller 5 min, `DisruptionSnapshot` indexé ligne + arrêt, `GET /disruptions` filtré sur l'instant, 13 tests). **Front, étape 1 faite** : gravité sur les pastilles (couleur + glyphe) et liste dépliable des titres dans le `LinePicker`. **Étape 2 à venir** : stations non desservies marquées sur la carte — demande d'étendre `/disruptions` pour résoudre les quais (`byStop`) en stations parentes. **Écarté** : tronçons en pointillés, le flux ne donne l'entre-deux-stations que dans du texte libre |
