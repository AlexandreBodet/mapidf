...
| SEC-5 | Secrets hors du code | `mapidf/mapidf` en dur dans `application.yml` et dans le compose. Cible : tous les secrets dans `.env` (déjà gitignoré, déjà lu par `docker compose`) et **plus aucune valeur par défaut** dans `application.yml`. Point à trancher : `./mvnw spring-boot:run` ne lit pas `.env` tout seul → soit `spring.config.import: optional:file:../.env[.properties]`, soit un fichier d'env côté IDE | S | P1 | à faire |
